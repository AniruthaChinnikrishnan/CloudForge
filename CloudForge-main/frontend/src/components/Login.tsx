  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token && typeof token === 'string' && token.trim().length > 0) {
        // Sanitize: strip any non-alphanumeric/dash/dot/underscore characters before storing
        const sanitizedToken = token.trim().replace(/[^a-zA-Z0-9\-._~+/]+=*/g, (match) => encodeURIComponent(match));
